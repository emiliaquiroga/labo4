...
function crearTabla(lista){
    ...
    lista.forEach(elemento => {
        ...
        <td><img src="${elemento.imagen || 'default.png'}" style="width: 50px; height: auto;"></td>
...
function agregarElemento() {
    ...
    let imagenLocal = document.getElementById("imagenLocal").files[0];
    let imagen = imagenUrl || imagenLocal ? imagenUrl || URL.createObjectURL(imagenLocal) : null;

    if (tipo === 'Auto') {
        let cantPuertas = document.getElementById("numCantPuertas").value;
        let asientos = document.getElementById("numAsientos").value;
        return new Auto(null, modelo, anoFabricacion, velMax, cantPuertas, asientos, imagen);
    } else if (tipo === 'Camion') {
        let carga = document.getElementById("numCarga").value;
        let autonomia = document.getElementById("numAutonomia").value;
        return new Camion(null, modelo, anoFabricacion, velMax, carga, autonomia, imagen);
    } else {
        alert("Error al instanciar el nuevo Vehículo!");
        return null;
    }
}
